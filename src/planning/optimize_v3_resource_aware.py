"""
TrackEase V3.12 - Resource-Aware Optimizer V3
...
This optimizer consumes ONLY windows that passed V3.11.
...
DATA_ORIGIN = "TRACKEASE_V3_RESOURCE_AWARE_OPTIMIZER"
INTEGRATION_MODE = "PROTOTYPE_CONSTRAINT_AWARE_GREEDY_OPTIMIZER"
...
def build_assignment_index(
    ...
    for row in assignments.to_dict(
        orient="records"
    ):
...
def is_exclusive_physical_resource(
    assignment: dict,
) -> bool:
...
def check_candidate_resources(
    ...
    proposed_bookings = []
    proposed_material = defaultdict(float)

    # Prevent the same physical resource being required twice by overlapping
    # requirements within one candidate.
    local_resource_intervals: dict[
        str,
        list[
            tuple[
                float,
    ...
    local_crew_duty = defaultdict(float)
    local_machine_minutes = defaultdict(float)
    ...
    for assignment in assignments:
        ...
        if not is_exclusive_physical_resource(
            assignment
        ):
            continue
        ...
        for (
            ...
            if cyclic_overlap(
                ...
                # Multiple requirements can legitimately select the same crew
                # for the same task. Treat identical task-time coverage as one
                # booking instead of a self-conflict.
                if (
                    math.isclose(
                        booking_start,
                        local_start,
    ...
    for (
        ...
        already_used = state.material_used[
            inventory_id
        ]
...
def main() -> None:
    print("=" * 72)
    print("TrackEase V3.12 - Resource-Aware Optimizer V3")
    print("=" * 72)
    ...
    print(
        f"V3.11 resource assignments  : {len(assignments):,}"
    )
    ...
    print(
        "V3.12 RESOURCE-AWARE OPTIMIZATION COMPLETE"
    )
